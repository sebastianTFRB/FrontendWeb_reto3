...
import { ArrowRight, ShieldCheck, Zap, BotMessageSquare, Home } from 'lucide-react'
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-black via-slate-950 to-black text-slate-50">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div className="absolute top-1/3 right-1/4 h-80 w-80 rounded-full bg-white/10 blur-3xl" />
      </div>
      ...
      <header className="relative mx-auto flex max-w-6xl items-center justify-between px-6 py-6">
        ...
        <div className="flex items-center gap-3 text-sm font-semibold">
          <Link
            to="/auth/login"
            className="rounded-full border border-red-600/40 px-4 py-2 text-slate-200 transition hover:border-red-400 hover:text-white"
          >
            Iniciar sesión
          </Link>
          <Link
      ...
      <main className="relative mx-auto grid max-w-6xl items-center gap-10 px-6 py-10 lg:grid-cols-2">
        <div className="space-y-6">
          <p className="text-xs uppercase tracking-[0.3em] text-red-300">Lead Agent</p>
          <h1 className="text-4xl font-semibold text-white sm:text-5xl">
            Encuentra, clasifica y prioriza leads inmobiliarios con IA
          </h1>
          <p className="max-w-xl text-lg text-slate-300">
            El agente detecta presupuesto, zona preferida y urgencia. Clasifica automáticamente en A/B/C y sugiere
            acciones para cerrar más rápido.
          </p>
          <div className="flex flex-wrap gap-3">
            ...
            <Link
              to="/app/dashboard"
              className="inline-flex items-center gap-2 rounded-full border border-red-600/30 px-5 py-3 text-sm font-semibold text-slate-200 transition hover:border-red-400 hover:text-white"
            >
              Ver dashboard
            </Link>
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              ...
              <div
                key={item.title}
                className="rounded-2xl border border-red-600/30 bg-black/40 p-3 backdrop-blur"
              >
        ...
        <div className="relative">
          ...
          <div className="relative rounded-3xl border border-red-600/30 bg-black/40 p-4 shadow-2xl shadow-red-900/20 backdrop-blur">
            <div className="flex items-center justify-between rounded-2xl border border-red-600/30 bg-black/40 px-4 py-3">
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-red-200">Demo</p>
                <p className="text-lg font-semibold text-white">Panel del agente</p>
              </div>
              <Link
                to="/auth/login"
                className="rounded-full bg-gradient-to-r from-red-600 to-red-700 px-4 py-2 text-sm font-semibold text-white"
              >
                Entrar
              </Link>
            </div>
            <div className="mt-4 space-y-3 rounded-2xl border border-red-600/30 bg-black/30 p-4">
              ...
              <div className="flex items-center justify-between text-sm text-slate-200">
                ...
                <span className="text-red-200">46 / 62 / 40</span>
              </div>
              <div className="flex items-center justify-between text-sm text-slate-200">
